...
- [x] 5.4 After successful flush, directly call `wal.purge_segments()` retaining latest 2 segments for the partition
...
- [x] 6.2 After WAL purge, evaluate `sstable_files.len()`; if it exceeds threshold (e.g., 8), trigger compaction inline
...
- [x] 7.3 After flush, truncate WAL via direct `wal.purge_segments()` call, retaining only the last 2 segments for the partition
- [x] 7.4 Implement `StoreEngine::shutdown()` for graceful shutdown — force flush all memtables synchronously
