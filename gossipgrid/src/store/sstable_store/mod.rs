...
use std::{collections::{BTreeMap, HashMap}, fs, ops::Bound, path::PathBuf};
...
#[allow(dead_code)]
pub(crate) struct SstableStore {
    partitions: BTreeMap<PartitionId, PartitionStore>,
    data_dir: PathBuf,
...
mod tests {
    ...
    fn test_partition_store_crud_and_count_across_memtable_and_sstables() {
        ...
        store.insert(&new_item, test_item("new-003", ItemStatus::Active)).unwrap();
        store.remove(
            &tombstoned,
            HLC {
                timestamp: 1,
