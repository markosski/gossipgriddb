...
// Keep old test for backward compatibility with /compute endpoint
#[tokio::test]
async fn test_compute_sum_legacy() {
    let _ = env_logger::try_init();
    ...
    let _ = client
        .post(format!("http://localhost:{port1}/items"))
        .body(r#"{"partition_key": "calc2", "range_key": "1", "message": "{\"v\": 10}"}"#)
        .send()
        .await
        .unwrap();
    ...
    let _ = client
        .post(format!("http://localhost:{port1}/items"))
        .body(r#"{"partition_key": "calc2", "range_key": "2", "message": "{\"v\": 20}"}"#)
        .send()
        .await
        .unwrap();
